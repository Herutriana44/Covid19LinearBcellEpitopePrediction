...
import sys
sys.path.append('.')
from models_logic.models import hmm_model
...
if __name__ == "__main__":
    ...
    for prop in prop_scales:
        for size in test_sizes:
            acc, auc = hmm_model(df, ['Position z-score', prop], 'label', size)
            results.append({'prop': prop, 'test_size': size, 'accuracy': acc, 'auc': auc})
    pd.DataFrame(results).to_csv('results/hmm_results.csv', index=False)
